...
values ('candidate-documents', 'candidate-documents', false, 10485760, array['application/pdf','image/jpeg','image/png','image/webp','application/msword','application/vnd.openxmlformats-officedocument.wordprocessingml.document'])
on conflict (id) do update set public = false, file_size_limit = excluded.file_size_limit, allowed_mime_types = excluded.allowed_mime_types;

drop policy if exists candidate_documents_owner_insert on storage.objects;
create policy candidate_documents_owner_insert on storage.objects for insert to authenticated
...
create policy candidate_documents_owner_read on storage.objects for select to authenticated
  using (bucket_id = 'candidate-documents' and ((storage.foldername(name))[1] = auth.uid()::text or public.ats_is_admin()));
drop policy if exists candidate_documents_owner_delete on storage.objects;
create policy candidate_documents_owner_delete on storage.objects for delete to authenticated
  using (bucket_id = 'candidate-documents' and ((storage.foldername(name))[1] = auth.uid()::text or public.ats_is_admin()));
