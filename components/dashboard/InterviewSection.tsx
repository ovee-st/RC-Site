...
export default function InterviewSection({ interviews }: { interviews: InterviewEvent[] }) {
  return (
    <Card className="h-[330px] overflow-hidden p-4">
      ...
      <div className="mt-4 grid max-h-[260px] gap-3 overflow-y-auto pr-1 lg:grid-cols-2">
        {interviews.map((interview) => (
          <div key={interview.id} className="rounded-2xl border border-border bg-bg p-3 dark:border-white/10 dark:bg-white/5">
            ...
            <div className="mt-4 flex gap-2"><Button type="button" onClick={() => window.open(interview.meetingUrl, "_blank")} className="gap-1.5 px-3 py-2 text-xs"><Video className="h-3.5 w-3.5" /> Join</Button><Button type="button" variant="secondary" className="gap-1.5 px-3 py-2 text-xs"><Clock className="h-3.5 w-3.5" /> Mock</Button></div>
          </div>
        ))}
      </div>
    </Card>
