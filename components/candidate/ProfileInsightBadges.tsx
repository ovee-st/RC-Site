...
export default function ProfileInsightBadges({ analysis, promptsUsed, promptLimit }: { analysis: ProfileAnalysis; promptsUsed: number; promptLimit: number }) {
  return (
    <div className="grid gap-2 sm:grid-cols-3">
      <div className="rounded-2xl border border-primary/15 bg-primary/5 p-3 dark:border-blue-400/20 dark:bg-blue-400/10">
        <p className="text-[10px] font-black uppercase tracking-wide text-text-muted dark:text-slate-300">Profile score</p>
        <p className="mt-1 text-xl font-black text-text-main dark:text-white">{analysis.profileCompletionScore}%</p>
      </div>
      <div className="rounded-2xl border border-success/15 bg-success/5 p-3 dark:bg-success/10">
        <p className="text-[10px] font-black uppercase tracking-wide text-text-muted dark:text-slate-300">ATS score</p>
        <p className="mt-1 text-xl font-black text-text-main dark:text-white">{analysis.atsScore}%</p>
      </div>
