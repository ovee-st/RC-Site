...
function AnalyticsView() {
  ...
  return (
    <div className="grid gap-6">
      ...
      <Card className="rounded-3xl p-5"><Badge variant="primary">Top issue categories</Badge><div className="mt-5 grid gap-3">{categories.map((category) => <div key={category} className="grid grid-cols-[minmax(7rem,0.7fr)_minmax(4rem,1fr)_auto] items-center gap-2 sm:gap-3"><span className="break-words text-sm font-bold text-text-muted">{category}</span><div className="h-2 rounded-full bg-bg dark:bg-white/10"><div className="h-full rounded-full bg-primary" style={{ width: `${Math.max(18, Math.min(100, tickets.filter((ticket) => (ticket.category || "Other") === category).length * 18))}%` }} /></div><span className="text-sm font-black text-text-main dark:text-white">{tickets.filter((ticket) => (ticket.category || "Other") === category).length}</span></div>)}</div></Card>
    </div>
  );
}
