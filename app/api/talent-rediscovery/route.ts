...
export async function GET(request: Request) {
  try {
    ...
    const url = new URL(request.url); const query = cleanText(url.searchParams.get("q"), 180); const limit = Math.max(1, Math.min(50, Number(url.searchParams.get("limit") || 25))); const page = Math.max(1, Number(url.searchParams.get("page") || 1));
    if (query.length < 2) return NextResponse.json({ results: [], query, semantic: false, page, limit, total: 0 });
    ...
    const applications = await applicationsQuery; if (applications.error) throw applications.error;
    ...
    const candidates = candidateIds.length ? await context.client.from("candidates").select("id,user_id,full_name,name,target_role,skills,skills_array,experience,about").or(`id.in.(${candidateIds.join(",")}),user_id.in.(${candidateIds.join(",")})`).limit(500) : { data: [], error: null };
    ...
    const results = (candidates.data || []).map((candidate) => {
      ...
      const ranked = scoreRediscoveryCandidate({ query, name: candidate.full_name || candidate.name || "Candidate", title: candidate.target_role, skills: candidateSkills, experience: `${candidate.experience || ""} ${candidate.about || ""}`, latestStage: application?.status, rejectionReason: application?.status?.toLowerCase().includes("reject") ? application.status : null, latestApplicationAt: application?.created_at });
      return { candidateId: candidate.id, applicationId: application?.id || null, name: candidate.full_name || candidate.name || "Candidate", title: candidate.target_role || null, skills: candidateSkills, experience: candidate.experience || null, latestStage: application?.status || null, latestApplicationAt: application?.created_at || null, rejectionReason: application?.status?.toLowerCase().includes("reject") ? application.status : null, ...ranked };
    }).filter((candidate) => candidate.score > 0).sort((a, b) => b.score - a.score);
    const from = (page - 1) * limit;
    return NextResponse.json({ results: results.slice(from, from + limit), query, semantic: false, page, limit, total: results.length, note: "Deterministic evidence ranking is active. A semantic score can be supplied when an embedding provider is configured." });
